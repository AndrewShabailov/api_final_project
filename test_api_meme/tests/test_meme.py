...
from data.payloads import upd_payload
...
@pytest.mark.positive
def test_token_is_alive(start_end_testing, before_after_testing, read_meme_endpoint):
    read_meme_endpoint.read_meme()
    read_meme_endpoint.read_token_is_alive()
    read_meme_endpoint.check_status_code_is_200()
...
@pytest.mark.positive
def test_update_meme(start_end_testing, before_after_testing, meme_factory, update_meme_endpoint):
    meme_id = meme_factory()
    assert meme_id is not None

    payload_to_send = upd_payload.copy()
    payload_to_send['id'] = int(meme_id)

    update_meme_endpoint.update_meme(payload=payload_to_send, meme_id=meme_id)
